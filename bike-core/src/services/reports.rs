...
pub struct TrainingReportPreparedRequest {
...
impl ReportsService {
    pub fn definitions() -> TrainingReportDefinitionsResponse {
    ...
    pub fn prepare_training_report_request(
        ...
        validate_report_filters(&request.query)?;
...
struct RideSummaryAccumulator {
    total_distance_meters: f64,
    total_elevation_gain_meters: f64,
    total_elapsed_seconds: i32,
    total_moving_seconds: i32,
    total_stopped_seconds: i32,
    weighted_hr_sum: f64,
    weighted_hr_seconds: i32,
    max_heart_rate_bpm: Option<i32>,
    zone_seconds: [i32; 5],
    missing_distance: usize,
    missing_elevation: usize,
    missing_time: usize,
    missing_hr: usize,
}

impl RideSummaryAccumulator {
    fn add_activity(&mut self, activity: &activities::Model) {
        match activity.distance_meters {
            Some(distance) => self.total_distance_meters += distance.max(0.0),
            None => self.missing_distance += 1,
        }

        match activity.elevation_gain_meters {
            Some(elevation) => self.total_elevation_gain_meters += elevation.max(0.0),
            None => self.missing_elevation += 1,
        }

        let moving_seconds = activity.moving_time_seconds.unwrap_or_default().max(0);
        let elapsed_seconds = activity_elapsed_seconds_from_summary(activity, moving_seconds);
        ...
        self.total_moving_seconds += moving_seconds;
        self.total_stopped_seconds += (elapsed_seconds - moving_seconds).max(0);
        self.add_heart_rate(activity, moving_seconds, elapsed_seconds);
        self.add_heart_rate_zones(activity);
    }
    ...
    fn add_heart_rate(
        ...
        if let Some(avg_hr) = activity.average_heart_rate_bpm {
            ...
            self.weighted_hr_sum += f64::from(avg_hr) * f64::from(weight);
            self.weighted_hr_seconds += weight;
        } else {
            self.missing_hr += 1;
        }

        if let Some(activity_max_hr) = activity.max_heart_rate_bpm {
            self.max_heart_rate_bpm = Some(
                self.max_heart_rate_bpm
                    .map_or(activity_max_hr, |current| current.max(activity_max_hr)),
            );
        }
    }

    fn add_heart_rate_zones(&mut self, activity: &activities::Model) {
        for zone in deserialize_activity_heart_rate_zones(activity.heart_rate_zones_json.as_ref()) {
            ...
            self.zone_seconds[zone_index] += zone.duration_seconds.max(0);
        }
    }

    fn data_quality_flags(&self, activity_count: usize) -> Vec<String> {
    ...
    fn into_response(self, activity_count: usize) -> RideSummaryReportResponse {
        let average_speed_mps = self.average_speed_mps();
        RideSummaryReportResponse {
            activity_count: activity_count as i32,
            total_distance_meters: round_metric(self.total_distance_meters),
            total_distance_miles: round_metric(self.total_distance_meters / 1609.344),
            total_elevation_gain_meters: round_metric(self.total_elevation_gain_meters),
            total_elevation_gain_feet: round_metric(
                self.total_elevation_gain_meters * FEET_PER_METER,
            ),
            total_elapsed_seconds: self.total_elapsed_seconds,
            total_moving_seconds: self.total_moving_seconds,
            total_stopped_seconds: self.total_stopped_seconds,
            average_speed_mps,
            average_speed_mph: average_speed_mps.map(|speed| round_metric(speed * 2.236_936)),
            average_heart_rate_bpm: average_or_none(self.weighted_hr_sum, self.weighted_hr_seconds),
            max_heart_rate_bpm: self.max_heart_rate_bpm,
            climbing_density_feet_per_hour: self.climbing_density_feet_per_hour(),
            z1_seconds: self.zone_seconds[0],
            z2_seconds: self.zone_seconds[1],
            z3_seconds: self.zone_seconds[2],
            z4_seconds: self.zone_seconds[3],
            z5_seconds: self.zone_seconds[4],
            data_quality_flags: self.data_quality_flags(activity_count),
        }
    }

    fn average_speed_mps(&self) -> Option<f64> {
        (self.total_moving_seconds > 0 && self.total_distance_meters > 0.0).then(|| {
            round_metric(self.total_distance_meters / f64::from(self.total_moving_seconds))
        })
    }

    fn climbing_density_feet_per_hour(&self) -> Option<f64> {
        let moving_hours = f64::from(self.total_moving_seconds) / 3600.0;
        (moving_hours > 0.0 && self.total_elevation_gain_meters > 0.0)
            .then(|| round_metric(self.total_elevation_gain_meters * FEET_PER_METER / moving_hours))
    }
}

fn activity_elapsed_seconds_from_summary(activity: &activities::Model, moving_seconds: i32) -> i32 {
    activity
        .total_time_seconds
        .or_else(|| {
            activity
...
impl BucketAccumulator {
    fn add_activity(
        &mut self,
        activity: &activities::Model,
        analysis: Option<&activity_training_analyses::Model>,
    ) {
        ...
        {
            self.z2_speed_sum += speed_mps;
            self.z2_speed_count += 1;
        }
        ...
        {
            self.decoupling_sum += decoupling_percent;
            self.decoupling_count += 1;
        }

        if let Some(climb_rate_meters_per_hour) = standalone_analysis.median_climb_rate {
            self.climb_rate_sum += climb_rate_meters_per_hour * FEET_PER_METER;
            self.climb_rate_count += 1;
        }
        ...
        let climbing_meters = analysis
            ...
            .unwrap_or(0.0);
        self.climbing_feet_total += climbing_meters * FEET_PER_METER;
        self.distance_meters_total += activity.distance_meters.unwrap_or(0.0);
        self.elevation_meters_total += activity
    ...
    fn into_point(
        ...
        let day_span =
            ((bucket_end - bucket_start).num_seconds() as f64 / 86_400.0).max(1.0 / 24.0);

        TrainingReportPointResponse {
            bucket_start: bucket_start.to_rfc3339(),
            bucket_end: bucket_end.to_rfc3339(),
            distance_meters: round_metric(self.distance_meters_total),
            distance_miles: round_metric(self.distance_meters_total / 1609.344),
            z2_average_speed_mps: average_or_none(self.z2_speed_sum, self.z2_speed_count),
            average_aerobic_decoupling_percent: average_or_none(
                self.decoupling_sum,
                self.decoupling_count,
            ),
            climbing_pace_feet_per_week: (self.climbing_feet_total > 0.0)
                .then(|| round_metric(self.climbing_feet_total * 7.0 / day_span)),
            climbing_vertical_rate_feet_per_hour: average_or_none(
                self.climb_rate_sum,
                self.climb_rate_count,
            ),
            z1_seconds: self.zone_seconds[0],
            z2_seconds: self.zone_seconds[1],
            z3_seconds: self.zone_seconds[2],
            z4_seconds: self.zone_seconds[3],
            z5_seconds: self.zone_seconds[4],
            elevation_gain_meters: round_metric(self.elevation_meters_total),
            elevation_gain_feet: round_metric(self.elevation_meters_total * FEET_PER_METER),
            activity_type_times: activity_type_time_rows(&self.activity_type_seconds),
        }
    }
}
...
impl TrainingReportEnvelope {
    fn from_context(context: &TrainingReportBuildContext) -> Self {
        Self {
    ...
    fn response(self) -> TrainingReportsResponse {
        TrainingReportsResponse {
            generated_at: self.generated_at,
            boundary: self.boundary,
            range_start: self.range_start.to_rfc3339(),
            range_end: self.range_end.to_rfc3339(),
            points: Vec::new(),
            ride_summary: None,
            endurance: None,
            climbing: None,
...
async fn build_training_reports(
    ...
    let envelope = TrainingReportEnvelope::from_context(&context);
    ...
    match report_id {
        ReportId::RideSummary => Ok(ride_summary_response(envelope, &activity_models)),
        ReportId::Endurance => Ok(endurance_response(envelope, &activity_models)),
        ReportId::Climbing => Ok(climbing_response(envelope, &activity_models)),
        ReportId::Fatigue => Ok(fatigue_response(envelope, &activity_models)),
        ReportId::CompareRides => {
            compare_rides_response(envelope, &context, &query, &activity_models).await
        }
        ReportId::Reassessment => reassessment_response(&context, &query).await,
        ReportId::Distance
        | ReportId::Elevation
...
fn ride_summary_response(
    envelope: TrainingReportEnvelope,
    activities: &[activities::Model],
) -> TrainingReportsResponse {
    let mut response = envelope.response();
    response.ride_summary = Some(build_ride_summary_report(activities));
    response
}

fn endurance_response(
    envelope: TrainingReportEnvelope,
    activities: &[activities::Model],
) -> TrainingReportsResponse {
    let mut response = envelope.response();
    response.endurance = Some(build_endurance_report(activities));
    response
}

fn climbing_response(
    envelope: TrainingReportEnvelope,
    activities: &[activities::Model],
) -> TrainingReportsResponse {
    let mut response = envelope.response();
    response.climbing = Some(build_climbing_report(activities));
    response
}
...
fn fatigue_response(
    ...
    let mut response = envelope.response();
    response.fatigue = Some(build_fatigue_report(activities));
    response
}

async fn compare_rides_response(
    envelope: TrainingReportEnvelope,
    context: &TrainingReportBuildContext,
    query: &TrainingReportsQuery,
    ...
    let mut response = envelope.response();
    response.compare_rides = Some(build_compare_rides_report(candidates, &selected_models));
    Ok(response)
}
...
async fn reassessment_response(
    ...
    let mut response = TrainingReportEnvelope {
        generated_at: context.now,
        boundary: context.boundary,
        range_start: range.reassessment_start,
        range_end: range.reassessment_end,
    }
    .response();
    response.reassessment = Some(build_reassessment_report(
        &activities,
        &fitness_rows,
        preferences.as_ref(),
        range.reassessment_start.date_naive(),
        range.reassessment_end.date_naive(),
    ));
...
#[derive(Debug, Clone, Copy)]
struct ReassessmentAnalysisRange {
    reassessment_start: DateTime<Utc>,
    reassessment_end: DateTime<Utc>,
...
fn reassessment_analysis_range(
    ...
    let reassessment_end = make_utc_datetime(
        ...
        context.now.date_naive().day(),
        23,
        59,
        59,
    )?;
    let reassessment_start = match preferences.and_then(|model| model.xc_goal_start_date) {
        ...
        None => context.range_start,
    };
    let spring_start = spring_baseline_start(reassessment_end.date_naive())?;

    Ok(ReassessmentAnalysisRange {
        reassessment_start,
        reassessment_end,
        analysis_start: reassessment_start.min(spring_start),
    })
}
...
async fn load_reassessment_activities(
    context: &TrainingReportBuildContext,
    query: &TrainingReportsQuery,
...
async fn build_aggregate_training_reports(
    ...
    let effective_range_start = aggregate_effective_range_start(&context, query, &activities);
    let points = aggregate_report_points(&context, effective_range_start, &buckets)?;

    Ok(TrainingReportsResponse {
        generated_at: context.now,
        boundary: context.boundary,
        range_start: effective_range_start.to_rfc3339(),
        range_end: context.range_end.to_rfc3339(),
        points,
        ride_summary: None,
        endurance: None,
        climbing: None,
...
fn aggregate_report_buckets(
    ...
    for activity in activities {
        ...
        buckets
            .entry(bucket_start)
            .or_default()
            .add_activity(activity, analysis_by_activity.get(&activity.id));
    }

    Ok(buckets)
}

fn aggregate_effective_range_start(
    context: &TrainingReportBuildContext,
    query: &TrainingReportsQuery,
    activities: &[activities::Model],
) -> DateTime<Utc> {
    if context.boundary == ReportBoundary::All
        ...
        && !activities.is_empty()
    {
        activities
            ...
            .min()
            .unwrap_or(context.range_start)
    } else {
        context.range_start
    }
}

fn aggregate_report_points(
    context: &TrainingReportBuildContext,
    ...
    while cursor <= context.range_end {
        let next_cursor = context.boundary.next_bucket_start(cursor)?;
        let bucket_end = if next_cursor > context.range_end {
            context.range_end
        } else {
            next_cursor
        };
        let accumulator = buckets.get(&cursor).cloned().unwrap_or_default();
        points.push(accumulator.into_point(cursor, bucket_end));
        cursor = next_cursor;
    }
...
fn report_definitions() -> Vec<TrainingReportDefinitionResponse> {
    vec![
        ride_summary_definition(),
        endurance_definition(),
        climbing_definition(),
        fatigue_definition(),
        compare_rides_definition(),
        reassessment_definition(),
        distance_definition(),
        elevation_definition(),
        activity_type_time_definition(),
        aggregate_trends_definition(),
    ]
}
...
struct ReportDefinitionSpec {
    ...
    short_purpose: &'static str,
    supported_filters: Vec<ReportFilterKey>,
    required_data_quality: Vec<&'static str>,
    result_sections: Vec<&'static str>,
    metrics: Vec<TrainingReportMetricDefinitionResponse>,
}

fn report_definition(spec: ReportDefinitionSpec) -> TrainingReportDefinitionResponse {
    TrainingReportDefinitionResponse {
        id: spec.id,
        display_name: spec.display_name.to_string(),
        short_purpose: spec.short_purpose.to_string(),
        supported_filters: spec.supported_filters,
        required_data_quality: string_list(spec.required_data_quality),
        result_sections: string_list(spec.result_sections),
        metrics: spec.metrics,
    }
}

fn string_list(values: Vec<&'static str>) -> Vec<String> {
    values.into_iter().map(str::to_string).collect()
}

fn standard_filters() -> Vec<ReportFilterKey> {
    vec![ReportFilterKey::MinDuration, ReportFilterKey::MinDistance]
}

fn ride_summary_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        ...
        supported_filters: standard_filters(),
        required_data_quality: vec!["distance", "elevation", "time", "heart_rate"],
        result_sections: vec!["summary_cards", "heart_rate_zones", "data_quality"],
        metrics: vec![
            metric_definition(
                "distance",
                "Distance",
...
fn endurance_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: ReportId::Endurance,
        display_name: "Endurance",
        short_purpose: "Aerobic durability, efficiency, speed, HR, and late-ride drift.",
        supported_filters: standard_filters(),
        required_data_quality: vec!["distance", "time", "heart_rate"],
        result_sections: vec!["summary_cards", "ride_table"],
        metrics: vec![
            metric_definition(
...
fn climbing_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: ReportId::Climbing,
        display_name: "Climbing",
        short_purpose: "Climb summaries, vertical rate trends, and raw climb rows.",
        supported_filters: standard_filters(),
        required_data_quality: vec!["distance", "elevation", "time"],
        result_sections: vec!["summary_cards", "climb_table"],
        metrics: vec![
            metric_definition(
                "longest_climb",
                "Longest Climb",
...
fn fatigue_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: ReportId::Fatigue,
        ...
        supported_filters: standard_filters(),
        required_data_quality: vec!["distance", "time", "heart_rate"],
        result_sections: vec!["ride_sections", "hourly_table"],
        metrics: vec![
            metric_definition(
                "average_speed",
                "Hourly Speed",
...
fn compare_rides_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: ReportId::CompareRides,
        display_name: "Compare Rides",
        ...
        required_data_quality: vec!["distance", "elevation", "time", "heart_rate"],
        result_sections: vec!["comparison_table", "candidate_table"],
        metrics: compare_ride_metric_definitions(),
    })
}

fn compare_ride_metric_definitions() -> Vec<TrainingReportMetricDefinitionResponse> {
    vec![
        metric_definition(
            "aerobic_decoupling_percent",
            "Aerobic Decoupling",
...
fn reassessment_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: ReportId::Reassessment,
        display_name: "Reassessment",
        short_purpose: "Reassess the active XC event goal from endurance progression, climbing density, elapsed long-ride pace, and spring-baseline fitness delta.",
        supported_filters: standard_filters(),
...
fn distance_definition() -> TrainingReportDefinitionResponse {
    bucket_definition(BucketDefinitionSpec {
        id: ReportId::Distance,
        display_name: "Distance",
        short_purpose: "Distance totals over the selected interval.",
        required_data_quality: "distance",
        metric_key: "distance",
        metric_label: "Distance",
        metric_unit: Some("mi"),
        direction: ReportMetricDirection::Neutral,
    })
}

fn elevation_definition() -> TrainingReportDefinitionResponse {
    bucket_definition(BucketDefinitionSpec {
        id: ReportId::Elevation,
        display_name: "Elevation",
        short_purpose: "Elevation gain totals over the selected interval.",
        required_data_quality: "elevation",
        metric_key: "elevation_gain",
        metric_label: "Elevation",
        metric_unit: Some("ft"),
        direction: ReportMetricDirection::Neutral,
    })
}

fn activity_type_time_definition() -> TrainingReportDefinitionResponse {
    bucket_definition(BucketDefinitionSpec {
        id: ReportId::ActivityTypeTime,
        display_name: "Time in Activity Type",
        short_purpose: "Moving time split between training and race activities.",
        required_data_quality: "time",
        metric_key: "activity_type_time",
        metric_label: "Activity Type Time",
        metric_unit: Some("hours"),
        direction: ReportMetricDirection::Neutral,
    })
}

struct BucketDefinitionSpec {
    id: ReportId,
    display_name: &'static str,
    short_purpose: &'static str,
    required_data_quality: &'static str,
...
fn bucket_definition(spec: BucketDefinitionSpec) -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: spec.id,
        display_name: spec.display_name,
        short_purpose: spec.short_purpose,
        supported_filters: standard_filters(),
        required_data_quality: vec![spec.required_data_quality],
        result_sections: vec!["bucket_chart"],
...
fn aggregate_trends_definition() -> TrainingReportDefinitionResponse {
    report_definition(ReportDefinitionSpec {
        id: ReportId::AggregateTrends,
        display_name: "Aggregate Trends",
        short_purpose: "Existing weekly, monthly, zone, climbing, and elevation charts.",
        supported_filters: standard_filters(),
        required_data_quality: vec!["distance", "elevation", "heart_rate_zones"],
        result_sections: vec!["bucket_charts"],
        metrics: vec![
            metric_definition(
                "z2_speed",
                "Z2 Speed",
...
fn build_ride_summary_report(activities: &[activities::Model]) -> RideSummaryReportResponse {
    let mut summary = RideSummaryAccumulator::default();
    for activity in activities {
        summary.add_activity(activity);
    }
    summary.into_response(activities.len())
}
...
fn build_compare_rides_report(
    ...
    let selected_sorted = selected_rides_by_start(selected);
    let analyses = compare_analyses_for_selected(&selected_sorted);

    CompareRidesReportResponse {
        candidates: compare_candidate_rows(candidates),
        selected_rides: compare_selected_ride_columns(&selected_sorted),
        metrics: compare_ride_metrics(&analyses),
    }
}

fn compare_candidate_rows(candidates: &[activities::Model]) -> Vec<CompareRideCandidateResponse> {
...
fn selected_rides_by_start(selected: &[activities::Model]) -> Vec<&activities::Model> {
    let mut selected_sorted = selected.iter().collect::<Vec<_>>();
    selected_sorted.sort_by_key(|activity| activity.started_at);
    selected_sorted
}

fn compare_selected_ride_columns(
    selected_sorted: &[&activities::Model],
) -> Vec<CompareRideColumnResponse> {
    selected_sorted
        .iter()
...
fn compare_analyses_for_selected(
    selected_sorted: &[&activities::Model],
) -> Vec<CompareRideAnalysis> {
    selected_sorted
        .iter()
...
fn compare_ride_metrics(analyses: &[CompareRideAnalysis]) -> Vec<CompareRideMetricResponse> {
    let mut metrics = compare_volume_metrics(analyses);
    metrics.extend(compare_effort_signal_metrics(analyses));
    metrics.extend(compare_context_metrics(analyses));
    metrics
}

fn compare_volume_metrics(analyses: &[CompareRideAnalysis]) -> Vec<CompareRideMetricResponse> {
    vec![
        compare_metric(
            "distance_miles",
            "Distance",
...
fn compare_effort_signal_metrics(
    analyses: &[CompareRideAnalysis],
) -> Vec<CompareRideMetricResponse> {
    vec![
        compare_metric(
            "aerobic_decoupling_percent",
            "Aerobic Decoupling",
...
fn compare_context_metrics(analyses: &[CompareRideAnalysis]) -> Vec<CompareRideMetricResponse> {
    vec![
        compare_metric(
...
struct ReassessmentWindows {
    recent: ReassessmentWindowMetrics,
    spring: ReassessmentWindowMetrics,
    prior_training: Option<ReassessmentWindowMetrics>,
}

struct ReassessmentSignals {
    endurance_progression: ReassessmentSignalResponse,
    climbing_density: ReassessmentSignalResponse,
    long_ride_pace: ReassessmentSignalResponse,
    fitness_delta: ReassessmentSignalResponse,
}

#[derive(Debug, Default)]
struct ReassessmentWindowAccumulator {
    activity_count: i32,
    total_distance_meters: f64,
    total_elevation_gain_meters: f64,
    benchmark_rides: Vec<ReassessmentBenchmarkRideResponse>,
...
impl ReassessmentWindowAccumulator {
    fn add_activity(&mut self, activity: &activities::Model) {
        self.activity_count += 1;
        self.total_distance_meters += activity.distance_meters.unwrap_or_default().max(0.0);
        self.total_elevation_gain_meters +=
            activity.elevation_gain_meters.unwrap_or_default().max(0.0);

        let samples = activity_samples(activity);
        let elapsed_seconds = activity_elapsed_seconds(activity, &samples);
        if is_reassessment_long_ride(activity, elapsed_seconds) {
            self.benchmark_rides.push(reassessment_benchmark_ride(
                activity,
                &samples,
                elapsed_seconds,
            ));
    ...
    fn sort_benchmark_rides(&mut self) {
        self.benchmark_rides.sort_by(|a, b| {
            b.elapsed_seconds
    ...
    fn response(
        ...
        let endurance_ride = reassessment_endurance_ride(&self.benchmark_rides);
        let pace_ride = reassessment_pace_ride(&self.benchmark_rides);
        let climbing_density_ride = reassessment_climbing_density_ride(&self.benchmark_rides);
        ...
        ReassessmentWindowResponse {
            ...
            long_ride_count: self.benchmark_rides.len() as i32,
            total_distance_miles: round_metric(self.total_distance_meters / 1609.344),
            total_elevation_gain_feet: round_metric(
                self.total_elevation_gain_meters * FEET_PER_METER,
            ),
            ...
            aggregate_long_ride_climbing_density_feet_per_hour:
                aggregate_climbing_density_feet_per_hour(&self.benchmark_rides),
            median_long_ride_decoupling_percent: median_reassessment_ride_metric(
                &self.benchmark_rides,
                |ride| ride.aerobic_decoupling_percent,
            ),
            median_long_ride_late_speed_change_percent: median_reassessment_ride_metric(
                &self.benchmark_rides,
                |ride| ride.late_speed_change_percent,
            ),
            median_long_ride_fatigue_index: median_reassessment_ride_metric(
                &self.benchmark_rides,
                |ride| ride.fatigue_index,
            ),
            average_fitness: mean(reassessment_fitness_values(
                fitness_rows,
                start_date,
                end_date,
            ))
            .map(round_metric),
            latest_fitness: latest_reassessment_fitness(fitness_rows, start_date, end_date)
                .map(round_metric),
        }
    }
}
...
fn build_reassessment_report(
    ...
    let windows = reassessment_windows(activities, fitness_rows, range_start, range_end);
    let improvement = reassessment_improvement(&windows.recent.response, &windows.spring.response);
    let ability_estimate = reassessment_ability_estimate(&windows.recent, &target);
    let signals = reassessment_signals(&windows, &target, fitness_rows, range_end);
    let verdict = reassessment_overall_verdict(&[
        signals.endurance_progression.status,
        signals.climbing_density.status,
        signals.long_ride_pace.status,
        signals.fitness_delta.status,
    ]);
    let (verdict_title, verdict_detail) = reassessment_verdict_copy(
        verdict,
        &target.response,
        &windows.recent.response,
        &windows.spring.response,
        &signals.fitness_delta,
        &ability_estimate,
    );
    let notes = reassessment_notes(&target, &windows.recent, &windows.spring);
    ...
    ReassessmentReportResponse {
        ...
        ability_estimate,
        recent_window: windows.recent.response.clone(),
        spring_baseline_window: windows.spring.response.clone(),
        improvement,
        endurance_progression: signals.endurance_progression,
        climbing_density: signals.climbing_density,
        long_ride_pace: signals.long_ride_pace,
        fitness_delta: signals.fitness_delta,
        benchmark_rides: reassessment_benchmark_rides(&windows),
        notes,
    }
}

fn reassessment_windows(
    activities: &[activities::Model],
    fitness_rows: &[fitness_freshness_daily::Model],
    range_start: NaiveDate,
    range_end: NaiveDate,
) -> ReassessmentWindows {
    let (spring_start, spring_end) = reassessment_spring_window(range_end);
    ...
    let prior_training = (training_start < current_start).then(|| {
        reassessment_window_metrics(
    ...
    ReassessmentWindows {
        recent: reassessment_window_metrics(
        ...
        spring: reassessment_window_metrics(
            "Spring baseline",
            spring_start,
            spring_end,
            activities,
...
fn reassessment_spring_window(range_end: NaiveDate) -> (NaiveDate, NaiveDate) {
    let spring_year = if range_end.month() < 6 {
        range_end.year() - 1
    } else {
        range_end.year()
...
fn reassessment_signals(
    windows: &ReassessmentWindows,
    target: &ReassessmentTarget,
    fitness_rows: &[fitness_freshness_daily::Model],
    range_end: NaiveDate,
) -> ReassessmentSignals {
    ReassessmentSignals {
        endurance_progression: reassessment_endurance_signal(
            &windows.recent,
            &windows.spring,
            windows.prior_training.as_ref(),
            target,
            fitness_rows,
            range_end,
        ),
        climbing_density: reassessment_climbing_density_signal(
            &windows.recent,
            &windows.spring,
            windows.prior_training.as_ref(),
            target,
        ...
        long_ride_pace: reassessment_long_ride_pace_signal(
            &windows.recent,
            &windows.spring,
            windows.prior_training.as_ref(),
            target,
        ...
        fitness_delta: reassessment_fitness_delta_signal(
            &windows.recent.response,
            &windows.spring.response,
            range_end,
        ),
...
fn reassessment_benchmark_rides(
    windows: &ReassessmentWindows,
) -> Vec<ReassessmentBenchmarkRideResponse> {
    let mut benchmark_rides = windows
        .recent
        .benchmark_rides
        .iter()
        .chain(windows.spring.benchmark_rides.iter())
        .cloned()
        .collect::<Vec<_>>();
...
fn reassessment_window_metrics(
    ...
    let mut accumulator = ReassessmentWindowAccumulator::default();

    for activity in activities
        .iter()
        .filter(|activity| (start_date..=end_date).contains(&activity.started_at.date_naive()))
    {
        accumulator.add_activity(activity);
    }

    accumulator.sort_benchmark_rides();
    let endurance_ride = reassessment_endurance_ride(&accumulator.benchmark_rides);
    let pace_ride = reassessment_pace_ride(&accumulator.benchmark_rides);
    let response = accumulator.response(label, start_date, end_date, fitness_rows);

    ReassessmentWindowMetrics {
        response,
        benchmark_rides: accumulator.benchmark_rides,
        endurance_ride,
        pace_ride,
    }
}

fn is_reassessment_long_ride(activity: &activities::Model, elapsed_seconds: i32) -> bool {
    elapsed_seconds >= REASSESSMENT_LONG_RIDE_MIN_SECONDS
        || activity
            .distance_meters
            .is_some_and(|distance| distance >= REASSESSMENT_LONG_RIDE_MIN_DISTANCE_METERS)
}

fn reassessment_benchmark_ride(
    activity: &activities::Model,
    samples: &[TrendSample],
    elapsed_seconds: i32,
    ...
    ReassessmentBenchmarkRideResponse {
        ...
        elapsed_speed_mph: elapsed_speed_mph(activity.distance_meters, elapsed_seconds),
        moving_speed_mph: analysis
            .moving_speed_mps
            .map(|speed| round_metric(speed * 2.236_936)),
        climbing_density_feet_per_hour: analysis.climbing_density_feet_per_hour.map(round_metric),
...
fn elapsed_speed_mph(distance_meters: Option<f64>, elapsed_seconds: i32) -> Option<f64> {
    distance_meters.and_then(|distance| {
        (distance > 0.0 && elapsed_seconds > 0)
            .then(|| round_metric((distance / f64::from(elapsed_seconds)) * 2.236_936))
    })
}

fn reassessment_endurance_ride(
    benchmark_rides: &[ReassessmentBenchmarkRideResponse],
) -> Option<ReassessmentBenchmarkRideResponse> {
    benchmark_rides
        .iter()
        .cloned()
...
fn reassessment_climbing_density_ride(
    benchmark_rides: &[ReassessmentBenchmarkRideResponse],
) -> Option<ReassessmentBenchmarkRideResponse> {
    benchmark_rides
        .iter()
...
fn median_reassessment_ride_metric<F>(
    benchmark_rides: &[ReassessmentBenchmarkRideResponse],
    value: F,
) -> Option<f64>
where
...
{
    median(benchmark_rides.iter().filter_map(value).collect()).map(round_metric)
}

fn reassessment_fitness_values(
    fitness_rows: &[fitness_freshness_daily::Model],
    start_date: NaiveDate,
    end_date: NaiveDate,
) -> Vec<f64> {
...
fn latest_reassessment_fitness(
    fitness_rows: &[fitness_freshness_daily::Model],
    start_date: NaiveDate,
    end_date: NaiveDate,
) -> Option<f64> {
...
fn reassessment_endurance_signal(
    ...
    reassessment_signal(ReassessmentSignalInput {
        status,
        ...
        current_value: duration_hours.map(round_metric),
        baseline_value: baseline_hours.map(round_metric),
        target_value: target_hours.map(round_metric),
        unit: "h",
        current_source: recent.endurance_ride.as_ref(),
        baseline_source: spring.endurance_ride.as_ref(),
        last_known_value,
        last_known_source,
...
fn reassessment_climbing_density_signal(
    ...
    let status = reassessment_climbing_density_status(current, baseline, target_value);
    let detail = reassessment_climbing_density_detail(status, current, baseline, target);
...
fn reassessment_climbing_density_status(
    current: Option<f64>,
    baseline: Option<f64>,
    target_value: Option<f64>,
) -> ReassessmentVerdict {
    let improved = percent_delta(current, baseline).unwrap_or_default() >= 15.0;
    if current.is_none() || baseline.is_none() {
        ReassessmentVerdict::MissingData
...
fn reassessment_climbing_density_detail(
...
fn reassessment_long_ride_pace_signal(
    ...
    reassessment_signal(ReassessmentSignalInput {
        status,
        title: "Long-ride pace",
        detail,
        current_value: current,
        baseline_value: baseline,
        target_value,
        unit: "mph",
        current_source: recent.pace_ride.as_ref(),
        baseline_source: spring.pace_ride.as_ref(),
        last_known_value,
        last_known_source,
        projected_current_value,
        projection_detail,
...
fn reassessment_fitness_delta_signal(
    ...
    reassessment_signal(ReassessmentSignalInput {
        status,
        title: "Fitness vs spring baseline",
        detail: match status {
            ReassessmentVerdict::OnTrack => {
...
struct ReassessmentSignalInput<'a> {
    status: ReassessmentVerdict,
    title: &'static str,
    detail: &'static str,
    ...
    target_value: Option<f64>,
    unit: &'static str,
    current_source: Option<&'a ReassessmentBenchmarkRideResponse>,
    baseline_source: Option<&'a ReassessmentBenchmarkRideResponse>,
    last_known_value: Option<f64>,
    last_known_source: Option<&'a ReassessmentBenchmarkRideResponse>,
    projected_current_value: Option<f64>,
    projection_detail: Option<String>,
    range_end: NaiveDate,
}

fn reassessment_signal(input: ReassessmentSignalInput<'_>) -> ReassessmentSignalResponse {
    ReassessmentSignalResponse {
        status: input.status,
        title: input.title.to_string(),
        detail: input.detail.to_string(),
        current_value: input.current_value.map(round_metric),
        projected_current_value: input.projected_current_value.map(round_metric),
        baseline_value: input.baseline_value.map(round_metric),
        target_value: input.target_value.map(round_metric),
        unit: input.unit.to_string(),
        current_source_activity_id: input.current_source.map(|ride| ride.activity_id),
        current_source_title: input.current_source.map(|ride| ride.title.clone()),
        current_source_started_at: input.current_source.map(|ride| ride.started_at),
        last_known_value: input.last_known_value.map(round_metric),
        last_known_source_activity_id: input.last_known_source.map(|ride| ride.activity_id),
        last_known_source_title: input.last_known_source.map(|ride| ride.title.clone()),
        last_known_source_started_at: input.last_known_source.map(|ride| ride.started_at),
        last_known_days_old: input.last_known_source.map(|ride| {
            (input.range_end - ride.started_at.date_naive())
                .num_days()
                .max(0)
        }),
        projection_detail: input.projection_detail,
        baseline_source_activity_id: input.baseline_source.map(|ride| ride.activity_id),
        baseline_source_title: input.baseline_source.map(|ride| ride.title.clone()),
        baseline_source_started_at: input.baseline_source.map(|ride| ride.started_at),
    }
}
...
const CLIMB_MIN_DURATION_SECONDS: i32 = 90;
const CLIMB_MIN_DISTANCE_METERS: f64 = 300.0;
const CLIMB_SUMMIT_CONFIRMATION_DROP_METERS: f64 = 5.0;
const CLIMB_VALLEY_RESET_DROP_METERS: f64 = 8.0;

#[derive(Debug, Clone, Copy)]
struct ClimbCandidate {
    valley_index: usize,
    valley_elevation: f64,
    crest_index: usize,
    crest_elevation: f64,
}

impl ClimbCandidate {
    fn new(index: usize, elevation: f64) -> Self {
        Self {
            valley_index: index,
    ...
    fn has_crest(self) -> bool {
        self.crest_index > self.valley_index
    }

    fn gain_meters(self) -> f64 {
        self.crest_elevation - self.valley_elevation
    }

    fn drop_from_crest(self, elevation: f64) -> f64 {
        self.crest_elevation - elevation
    }

    fn advance_crest(&mut self, index: usize, elevation: f64) {
        self.crest_index = index;
        self.crest_elevation = elevation;
    }

    fn reset(&mut self, index: usize, elevation: f64) {
        *self = Self::new(index, elevation);
    }
}

struct ClimbBuildContext<'a> {
    activity: &'a activities::Model,
    samples: &'a [TrendSample],
    total_elapsed_seconds: i32,
}
...
fn detect_climbs(activity: &activities::Model, samples: &[TrendSample]) -> Vec<ClimbResponse> {
    ...
    let total_elapsed = samples
        ...
        .unwrap_or_default();
    let context = ClimbBuildContext {
        activity,
        samples,
        total_elapsed_seconds: total_elapsed,
    };
    let Some((first_index, first_elevation)) = first_elevation_sample(samples) else {
        return climbs;
    };
    let mut candidate = ClimbCandidate::new(first_index, first_elevation);

    for (index, sample) in samples.iter().enumerate().skip(candidate.valley_index + 1) {
        let Some(elevation) = sample.elevation_meters else {
            continue;
        };

        process_climb_sample(&context, index, elevation, &mut candidate, &mut climbs);
    }

    finalize_climb(&context, candidate, &mut climbs);

    climbs
}

fn process_climb_sample(
    context: &ClimbBuildContext<'_>,
    index: usize,
    elevation: f64,
    candidate: &mut ClimbCandidate,
    climbs: &mut Vec<ClimbResponse>,
) {
    if elevation < candidate.valley_elevation {
        process_climb_descent(context, index, elevation, candidate, climbs);
    } else if elevation > candidate.crest_elevation {
        candidate.advance_crest(index, elevation);
    } else if candidate.drop_from_crest(elevation) >= CLIMB_SUMMIT_CONFIRMATION_DROP_METERS {
        finalize_climb(context, *candidate, climbs);
        candidate.reset(index, elevation);
    }
}

fn process_climb_descent(
    context: &ClimbBuildContext<'_>,
    index: usize,
    elevation: f64,
    candidate: &mut ClimbCandidate,
    ...
    let drop_from_crest = candidate.drop_from_crest(elevation);
    if candidate.has_crest() && drop_from_crest >= CLIMB_SUMMIT_CONFIRMATION_DROP_METERS {
        finalize_climb(context, *candidate, climbs);
        candidate.reset(index, elevation);
    } else if !candidate.has_crest() || drop_from_crest >= CLIMB_VALLEY_RESET_DROP_METERS {
        candidate.reset(index, elevation);
    }
}
...
fn finalize_climb(
    ...
    if candidate.crest_index <= candidate.valley_index
        || candidate.crest_index >= context.samples.len()
    {
        return;
    }

    let segment = &context.samples[candidate.valley_index..=candidate.crest_index];
    ...
    let gain_meters = candidate.gain_meters();

    if duration_seconds < CLIMB_MIN_DURATION_SECONDS
        || distance_meters < CLIMB_MIN_DISTANCE_METERS
        || gain_meters < CLIMB_MIN_GAIN_METERS
    {
        return;
    }
    ...
    climbs.push(ClimbResponse {
        ...
        activity_title: context.activity.title.clone(),
        ...
        heart_rate_recovery_30_seconds_bpm: heart_rate_recovery(
            context.samples,
            candidate.crest_index,
            30,
        ),
        heart_rate_recovery_60_seconds_bpm: heart_rate_recovery(
            context.samples,
            candidate.crest_index,
            60,
        ),
        seconds_to_drop_10_bpm: seconds_to_drop_bpm(context.samples, candidate.crest_index, 10),
        seconds_to_drop_15_bpm: seconds_to_drop_bpm(context.samples, candidate.crest_index, 15),
        summit_immediately_enters_descent: summit_enters_descent(
            context.samples,
            candidate.crest_index,
        ),
        first_or_second_half: if segment.last().unwrap().elapsed_seconds
            <= context.total_elapsed_seconds / 2
        {
            "first".to_string()
        } else {
            "second".to_string()
