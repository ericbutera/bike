...
pub struct TrainingGoalsService;

impl TrainingGoalsService {
    pub async fn xc_goal_progress(
        ...
        let activity_ids = analysis_models
            ...
            .collect::<Vec<_>>();
        let activity_by_id = load_activity_summaries_by_ids(db, user_id, &activity_ids).await?;

        let mut rides = xc_rides_from_analysis_models(analysis_models, &activity_by_id);
        let race_activities = load_race_activity_summaries(db, user_id).await?;

        append_missing_race_rides(&mut rides, race_activities);
        sort_xc_rides_recent_first(&mut rides);

        Ok(build_xc_goal_progress_response(rides, goal, freshness, now))
    }

    pub async fn dh_goal_progress(
        db: &DatabaseConnection,
        user_id: i32,
        now: DateTime<Utc>,
        ...
        let activity_ids = effort_models
            ...
            .collect::<Vec<_>>();
        let activity_by_id = load_activity_summaries_by_ids(db, user_id, &activity_ids).await?;
        let efforts = dh_effort_sources_from_models(effort_models, &activity_by_id);

        Ok(build_dh_goal_progress_response(
            segment_models,
            efforts,
            freshness,
            now,
...
fn xc_rides_from_analysis_models(
    analysis_models: Vec<activity_training_analyses::Model>,
    activity_by_id: &HashMap<i32, ActivitySummaryRow>,
) -> Vec<XcRideProgressResponse> {
    analysis_models
        .into_iter()
        .filter_map(|analysis| xc_ride_from_analysis_model(analysis, activity_by_id))
        .collect()
}

fn xc_ride_from_analysis_model(
    analysis: activity_training_analyses::Model,
    activity_by_id: &HashMap<i32, ActivitySummaryRow>,
) -> Option<XcRideProgressResponse> {
...
fn append_missing_race_rides(
    rides: &mut Vec<XcRideProgressResponse>,
    race_activities: Vec<ActivitySummaryRow>,
) {
    let ride_ids = rides
        .iter()
...
fn sort_xc_rides_recent_first(rides: &mut [XcRideProgressResponse]) {
    rides.sort_by_key(|ride| std::cmp::Reverse(ride.started_at));
}

fn dh_effort_sources_from_models(
    effort_models: Vec<segment_efforts::Model>,
    activity_by_id: &HashMap<i32, ActivitySummaryRow>,
) -> Vec<DhEffortSource> {
    effort_models
        .into_iter()
        .filter_map(|effort| dh_effort_source_from_model(effort, activity_by_id))
        .collect()
}

fn dh_effort_source_from_model(
    effort: segment_efforts::Model,
    activity_by_id: &HashMap<i32, ActivitySummaryRow>,
) -> Option<DhEffortSource> {
...
struct XcRecentTrainingMetrics {
    summary: XcProgressSummaryResponse,
    comparable_ride_count: usize,
...
fn build_xc_goal_progress_response(
    ...
    prepare_xc_rides(&mut rides, goal.as_ref());
    let race_results = build_xc_race_results(&rides);
    let training_rides = xc_training_rides(&rides);
    let event_goal = goal
        .as_ref()
        .and_then(|goal| build_xc_event_goal_response(&training_rides, goal, now));
    let recent_metrics = xc_recent_training_metrics(&training_rides, now);
    let goals = xc_goal_metrics(&recent_metrics);
    let (readiness, deficits) = goal
        ...
        .map(|goal| {
            build_xc_readiness(
                ...
                goal,
                now,
                recent_metrics.recent_decoupling_average,
                freshness,
            )
        })
    ...
    let recommendations = if goal.is_some() {
        build_xc_event_recommendations(
            &deficits,
            &training_rides,
            recent_metrics.comparable_ride_count,
            recent_metrics.weekly_z2_average_seconds,
            recent_metrics.weekly_climbing_average_meters,
            recent_metrics.recent_decoupling_average,
            freshness,
        )
    } else {
    ...
    XcGoalProgressResponse {
        ...
        recommendations,
        weekly_progress,
        recent_rides: xc_recent_progress_rides(training_rides, goal.as_ref(), now),
    }
}

fn prepare_xc_rides(rides: &mut [XcRideProgressResponse], goal: Option<&XcEventGoal>) {
    rides.sort_by_key(|ride| std::cmp::Reverse(ride.started_at));
    for ride in rides {
        ride.aerobic_decoupling_percent = ride
            .aerobic_decoupling_percent
            .and_then(plausible_aerobic_decoupling_percent);
        apply_xc_training_purpose(ride, goal);
    }
}

fn xc_training_rides(rides: &[XcRideProgressResponse]) -> Vec<XcRideProgressResponse> {
    rides
        .iter()
...
fn xc_recent_training_metrics(
    training_rides: &[XcRideProgressResponse],
    now: DateTime<Utc>,
) -> XcRecentTrainingMetrics {
    ...
    let recent_rides = training_rides
        ...
        .collect::<Vec<_>>();
    let comparable_ride_count = recent_rides
        .iter()
    ...
    XcRecentTrainingMetrics {
        summary: XcProgressSummaryResponse {
            ...
            comparable_ride_count: comparable_ride_count as i32,
            total_z2_time_seconds,
            total_climbing_time_seconds,
            total_climbing_elevation_gain_meters: round_metric(
                total_climbing_elevation_gain_meters,
            ),
            average_aerobic_decoupling_percent: recent_decoupling_average.map(round_metric),
        },
        comparable_ride_count,
        weekly_z2_average_seconds: f64::from(total_z2_time_seconds) / weekly_divisor,
        weekly_climbing_average_meters: total_climbing_elevation_gain_meters / weekly_divisor,
        recent_decoupling_average,
    }
}

fn xc_goal_metrics(metrics: &XcRecentTrainingMetrics) -> Vec<TrainingGoalMetricResponse> {
    vec![
        build_goal_metric(
            ...
            Some(metrics.weekly_z2_average_seconds),
        ...
        build_goal_metric(
            ...
            TrainingGoalDirection::AtLeast,
            Some(metrics.weekly_climbing_average_meters),
        ...
        build_goal_metric(
            ...
            metrics.recent_decoupling_average,
            XC_AEROBIC_DECOUPLING_GOAL_PERCENT,
        ),
    ]
}

fn xc_recent_progress_rides(
    training_rides: Vec<XcRideProgressResponse>,
    goal: Option<&XcEventGoal>,
    now: DateTime<Utc>,
) -> Vec<XcRideProgressResponse> {
...
struct XcReadinessMetrics {
    best_distance_meters: Option<f64>,
    best_climbing_meters: Option<f64>,
    current_climb_density: Option<f64>,
    target_climb_density: Option<f64>,
    target_finish_speed: Option<f64>,
    recent_z2_speed: Option<f64>,
}
...
fn build_xc_readiness(
    ...
    let metrics = xc_readiness_metrics(rides, goal, now);
    let gates = xc_readiness_gates(&metrics, goal, recent_decoupling_average, freshness);
    let deficits = sorted_xc_deficits(&gates, goal, now);
    (Some(xc_readiness_summary(gates, &deficits)), deficits)
}

fn xc_readiness_metrics(
    rides: &[XcRideProgressResponse],
    goal: &XcEventGoal,
    now: DateTime<Utc>,
) -> XcReadinessMetrics {
    ...
    let counted_climbing_meters = training_counted_rides
        ...
        .sum::<f64>();
    let benchmark_rides = xc_readiness_benchmark_rides(&counted_rides, now);
    ...
    XcReadinessMetrics {
        best_distance_meters: benchmark_rides
            .iter()
            .filter_map(|ride| ride.distance_meters)
            .reduce(f64::max),
        best_climbing_meters: benchmark_rides
            .iter()
            .filter_map(|ride| effective_climbing_elevation_gain_meters(ride))
            .reduce(f64::max),
        current_climb_density: climb_density_meters_per_kilometer(
            Some(counted_climbing_meters),
            Some(counted_distance_meters),
        ),
        target_climb_density: climb_density_meters_per_kilometer(
            Some(readiness_target_elevation_gain_meters),
            Some(goal.target_distance_meters),
        ),
        target_finish_speed: goal
            .target_finish_time_seconds
            .and_then(|seconds| target_finish_speed_mps(goal.target_distance_meters, seconds)),
        recent_z2_speed: weighted_z2_speed_mps(recent_rides.iter().copied()),
    }
}

fn xc_readiness_benchmark_rides<'a>(
    counted_rides: &[&'a XcRideProgressResponse],
    now: DateTime<Utc>,
) -> Vec<&'a XcRideProgressResponse> {
...
fn xc_readiness_gates(
    metrics: &XcReadinessMetrics,
    goal: &XcEventGoal,
    recent_decoupling_average: Option<f64>,
    freshness: Option<FitnessFreshnessSnapshot>,
) -> Vec<XcReadinessGateResponse> {
    let mut gates = vec![
        build_at_least_gate(XcReadinessGateInput {
            key: XcReadinessGateKey::LongRideDistance,
            label: "Recent long ride",
            current_value: metrics.best_distance_meters,
            target_value: goal.target_distance_meters * XC_LONG_RIDE_TARGET_RATIO,
            unit: TrainingMetricUnit::Meters,
            on_track_ratio: 1.0,
            watch_ratio: 0.75,
            detail: "Best single ride in the last 90 days, or any race in the saved block, compared with the long-ride benchmark for this event distance.",
        }),
        build_at_least_gate(XcReadinessGateInput {
            key: XcReadinessGateKey::BigClimbDay,
            label: "Recent climb day",
            current_value: metrics.best_climbing_meters,
            target_value: big_climb_day_target_meters(goal),
            unit: TrainingMetricUnit::Meters,
            on_track_ratio: 1.0,
            watch_ratio: 0.75,
            detail: "Best single climbing ride in the last 90 days, or any race in the saved block, compared with the climbing-day benchmark for this event.",
        }),
        build_at_least_gate(XcReadinessGateInput {
            key: XcReadinessGateKey::ClimbDensity,
            label: "Climb density",
            current_value: metrics.current_climb_density,
            target_value: metrics.target_climb_density.unwrap_or_default()
                * XC_CLIMB_DENSITY_READY_RATIO,
            unit: TrainingMetricUnit::MetersPerKilometer,
            on_track_ratio: 1.0,
            watch_ratio: XC_CLIMB_DENSITY_WATCH_RATIO / XC_CLIMB_DENSITY_READY_RATIO,
            detail: "Training-block climbing per distance compared with the event's climb density.",
        }),
        build_at_most_gate(XcReadinessGateInput {
            key: XcReadinessGateKey::AerobicDecoupling,
            label: "Aerobic decoupling",
            current_value: recent_decoupling_average,
            target_value: XC_AEROBIC_DECOUPLING_GOAL_PERCENT,
            unit: TrainingMetricUnit::Percent,
            on_track_ratio: 1.0,
            watch_ratio: 1.4,
    ...
    if let Some(target_finish_speed) = metrics.target_finish_speed {
        gates.push(build_at_least_gate(XcReadinessGateInput {
            key: XcReadinessGateKey::TargetFinishPace,
            label: "Target finish pace",
            current_value: metrics.recent_z2_speed,
            target_value: target_finish_speed * XC_FINISH_SPEED_READY_RATIO,
            unit: TrainingMetricUnit::MetersPerSecond,
            on_track_ratio: 1.0,
            watch_ratio: XC_FINISH_SPEED_WATCH_RATIO / XC_FINISH_SPEED_READY_RATIO,
            detail:
...
fn xc_readiness_summary(
    gates: Vec<XcReadinessGateResponse>,
...
fn build_at_least_gate(input: XcReadinessGateInput) -> XcReadinessGateResponse {
    let status = match input.current_value {
        Some(current)
            if input.target_value > 0.0 && current >= input.target_value * input.on_track_ratio =>
        {
            XcReadinessStatus::OnTrack
        }
        Some(current)
            if input.target_value > 0.0 && current >= input.target_value * input.watch_ratio =>
        {
    ...
    XcReadinessGateResponse {
        ...
        target_value: Some(round_metric(input.target_value)),
        gap_value: input
            .current_value
            .map(|current| (input.target_value - current).max(0.0))
            .map(round_metric),
...
fn build_at_most_gate(input: XcReadinessGateInput) -> XcReadinessGateResponse {
    let status = match input.current_value {
        Some(current)
            if input.target_value > 0.0 && current <= input.target_value * input.on_track_ratio =>
        {
    ...
    XcReadinessGateResponse {
        ...
        gap_value: input
            .current_value
            .map(|current| (current - input.target_value).max(0.0))
            .map(round_metric),
        progress_percent: goal_progress_percent(
            input.current_value,
            input.target_value,
...
struct XcRacePriorTraining {
    window_ride_count: usize,
    z2_time_seconds: i32,
    climbing_elevation_gain_meters: f64,
    average_z2_speed_mps: Option<f64>,
    average_aerobic_decoupling_percent: Option<f64>,
    best_distance_meters: Option<f64>,
    best_elevation_gain_meters: Option<f64>,
}
...
fn build_xc_race_results(rides: &[XcRideProgressResponse]) -> Vec<XcRaceResultResponse> {
    ...
    races
        .into_iter()
        .take(5)
        .map(|race| build_xc_race_result(race, rides))
        .collect()
}

fn build_xc_race_result(
    race: &XcRideProgressResponse,
    rides: &[XcRideProgressResponse],
) -> XcRaceResultResponse {
    let prior_training = xc_race_prior_training(race, rides);
    let race_elevation_gain_meters = effective_climbing_elevation_gain_meters(race);
    let distance_percent =
        relative_percent(race.distance_meters, prior_training.best_distance_meters);
    let elevation_percent = relative_percent(
        race_elevation_gain_meters,
        prior_training.best_elevation_gain_meters,
    );
    let (insight_title, insight_detail) = race_insight(
        prior_training.window_ride_count,
        prior_training.z2_time_seconds,
        prior_training.climbing_elevation_gain_meters,
        prior_training.average_aerobic_decoupling_percent,
        distance_percent,
        elevation_percent,
    );
    ...
    XcRaceResultResponse {
        ...
        climbing_elevation_gain_meters: race.climbing_elevation_gain_meters.map(round_metric),
        aerobic_decoupling_percent: race.aerobic_decoupling_percent.map(round_metric),
        prior_training_ride_count: prior_training.window_ride_count as i32,
        prior_training_z2_time_seconds: prior_training.z2_time_seconds,
        prior_training_climbing_elevation_gain_meters: round_metric(
            prior_training.climbing_elevation_gain_meters,
        ),
        prior_training_average_z2_speed_mps: prior_training.average_z2_speed_mps.map(round_metric),
        prior_training_average_aerobic_decoupling_percent: prior_training
            .average_aerobic_decoupling_percent
            .map(round_metric),
        race_vs_best_training_distance_percent: distance_percent.map(round_metric),
        race_vs_best_training_elevation_percent: elevation_percent.map(round_metric),
        insight_title,
        insight_detail,
    }
}

fn xc_race_prior_training(
    race: &XcRideProgressResponse,
    rides: &[XcRideProgressResponse],
) -> XcRacePriorTraining {
    ...
    XcRacePriorTraining {
        window_ride_count: prior_window_rides.len(),
        z2_time_seconds: prior_window_rides
            .iter()
            .map(|ride| ride.z2_time_seconds)
            .sum(),
        climbing_elevation_gain_meters: prior_window_rides
            .iter()
            .filter_map(|ride| effective_climbing_elevation_gain_meters(ride))
            .sum(),
        average_z2_speed_mps: average_f64(
            prior_window_rides
                .iter()
        ...
        best_distance_meters: prior_training_rides
            .iter()
            .filter_map(|ride| ride.distance_meters)
            .reduce(f64::max),
        best_elevation_gain_meters: prior_training_rides
            .iter()
            .filter_map(|ride| effective_climbing_elevation_gain_meters(ride))
            .reduce(f64::max),
    }
