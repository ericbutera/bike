...
export default function SegmentDetailEffortsSection({
  ...
  const showAttemptColumn = Array.from(attemptSummaryByEffortId.values()).some(
  ...
  return (
    <AppCard bodyClassName="gap-3">
            ...
            {EFFORT_TIME_FILTERS.map((filter) => (
              <button
                ...
                className={`join-item btn btn-sm ${
                  effortTimeFilter === filter.key ? "btn-neutral" : "btn-ghost"
                }`}
                onClick={() => {
      ...
      {isLoading ? (
        <div className="mt-5 flex min-h-[14rem] items-center justify-center bg-base-100">
          <LoadingSpinner size="md" aria-label="Loading segment efforts" />
        </div>
