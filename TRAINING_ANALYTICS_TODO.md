...
- [x] XC event targets now use a saved training start date through the target date so qualifying XC rides in that block inform the target context, visible ride count, weekly history, Z2 speed trend, climb-rate trend, and event-specific readiness gates.
